...
// New component for list view rendering of a patient
const PatientListItem = ({ loading, patient }) => {
    const navigate = useNavigate();

    if (loading) {
        return (
            <div className="bg-white rounded-lg shadow-sm p-4 animate-pulse flex items-center">
                <div className="w-10 h-10 bg-gray-200 rounded-full flex-shrink-0 mr-4"></div>
    ...
    return (
        <div className="bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100 group">
            <div className="p-4 flex items-center">
                ...
                <div className="flex items-center space-x-4 flex-grow">
                    <div className="relative">
                        <Avatar
                            ...
                            lastName={patient?.last_name || ''}
                            size={40}
                            className="flex-shrink-0 ring-2 ring-white shadow"
                        />
                    ...
                    <div>
                        ...
                        <div className="flex items-center text-xs text-gray-500 mt-1 space-x-4">
                            ...
                            <span className="flex items-center">
                                <svg className="h-3 w-3 mr-1 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                </svg>
                                <span>Dr. {patient.attending_physician}</span>
                            </span>
                            ...
                            {patient.admissions?.[0]?.diagnosis && (
                                <span className="flex items-center max-w-xs truncate">
                                    <svg className="h-3 w-3 mr-1 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                    </svg>
                                    <span className="truncate">{patient.admissions[0].diagnosis}</span>
                                </span>
                            )}
                        </div>
                    </div>
                ...
                {/* Status */}
                <span className={`px-3 py-1 text-xs font-semibold rounded-full border flex items-center ${getStatusColor(patient.admissions?.[0]?.status)} mr-4`}>
                    {getStatusIcon(patient.admissions?.[0]?.status)}
                    {patient.admissions?.[0]?.status || 'Unknown'}
                </span>
                ...
                <div className="flex items-center space-x-2">
                    <button
                        ...
                        className="text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50 px-3 py-1 rounded-md text-sm font-medium flex items-center transition-all duration-200"
                    >
                    ...
                    <button
                        ...
                        className="bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1 rounded-md text-sm font-medium flex items-center transition-all duration-200"
                    >
                        <svg className="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
...
// View toggle component for switching between card and list view
...
const PatientList = () => {
    ...
    // Render patients based on the selected view mode
    const renderPatients = () => {
        if (loading) {
            // Render loading skeleton based on view mode
            return viewMode === 'grid' 
                ? [...Array(9)].map((_, index) => <PatientCard key={index} loading={true} />)
                : [...Array(9)].map((_, index) => <PatientListItem key={index} loading={true} />);
        }
        ...
        return patients.map((patient) => (
            viewMode === 'grid' 
                ...
                : <PatientListItem key={patient.id} loading={false} patient={patient} />
        ));
    };
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="flex flex-col space-y-6 mb-8">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
                        <div className="relative max-w-full md:w-2/3">
                            <input
                                type="text"
                ...
                {/* Render patients with conditional container classes based on view mode */}
