...
const PatientAdmission = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Changed from /api/patients/admit to /patients/admit
            const response = await axiosClient.post('/patients/admit', formData);
            const { data } = response;

            if (data.id) {
                // Changed from /api/patients/${data.id}/qr to /patients/${data.id}/qr
            ...
            {qrCode && (
                <div className="mt-6 bg-white p-6 rounded-lg shadow-sm text-center">
                    ...
                    <div className="flex justify-center mb-4">
                        <img src={qrCode} alt="Patient QR Code" className="max-w-[200px]" />
                    </div>
